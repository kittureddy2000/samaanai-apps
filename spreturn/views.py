...
def spreturn(request):
    """
    View for S&P 500 return calculator.
    Calculates historical S&P returns based on user input parameters.
    """
    logger.info("Accessing S&P Return calculator")
    ...
    if request.method == "POST":
        logger.debug("Processing POST request for S&P Return calculator")
        form = SPReturnForm(request.POST)
        if form.is_valid():
            ...
            logger.debug(f"Form valid with years: {number_of_year}, recurring: {recurring_deposit}")
        else:
            logger.warning("Form invalid, using default values")
        ...
        logger.debug("Initial GET request for S&P Return calculator")
        form = SPReturnForm(initial=initial_values)
        number_of_year = initial_values["numYears"]
        recurring_deposit = initial_values["reccuringDeposit"]
        starting_investment = initial_values["startingInvest"]
        recurring_deposit_amount = initial_values["reccuringDepositAmount"]
    
    # Fetch S&P return data
    try:
        ...
        if not spreturn_data:
            logger.error("No S&P return data found in database")
            return render(request, 'spreturn/spreturn.html', {
                'error': 'No S&P return data available.',
                'form': form  # Include the form in the context
            })
        
        logger.info(f"Retrieved {len(spreturn_data)} S&P return records")
        
        # Create dictionaries for calculations
        ...
        series_sp_DD = []
        ReturnInfo = {}
        history_begin_year = min(SP500.keys())
        
        # Convert inputs to Decimal for precision
        starting_investment = Decimal(starting_investment)
        recurring_deposit_amount = Decimal(recurring_deposit_amount)
        
        # Calculate returns for each starting year
        for start_year in range(history_begin_year, 2024 - number_of_year):
            logger.debug(f"Calculating returns starting from year {start_year}")
            
            yearly_return_DD = starting_investment
            yearly_return_SP = starting_investment
            principal = starting_investment
            
            # Calculate compounded returns over period
            try:
                for year_offset in range(0, number_of_year):
                    current_year = start_year + year_offset
                    
                    if current_year not in SP500 or current_year not in SP500_dividend:
                        logger.warning(f"Missing data for year {current_year}, skipping")
                        continue
                        
                    sp_return_DD = Decimal(SP500_dividend[current_year])
                    sp_return = Decimal(SP500[current_year])
                    
                    if recurring_deposit == 'Yes':
                ...
                CGAR_DD = ((yearly_return_DD / starting_investment) ** (Decimal(1) / Decimal(number_of_year)) - Decimal(1))
                CGAR_SP = ((yearly_return_SP / starting_investment) ** (Decimal(1) / Decimal(number_of_year)) - Decimal(1))
                
                # Store results
                ReturnInfo[start_year] = [
                    end_year, 
                    round(yearly_return_SP, 2), 
                    round(yearly_return_DD, 2), 
                    round(CGAR_SP * 100, 2), 
                    round(CGAR_DD * 100, 2)
                ]
                ...
                logger.error(f"Error calculating returns for year {start_year}: {str(e)}")
                continue
        ...
        sp_return_stats = {
            'sp_min': sp_df["SP_Return"].min() if not sp_df.empty else 0,
            'sp_max': sp_df["SP_Return"].max() if not sp_df.empty else 0,
            'sp_mean': round(sp_df["SP_Return"].mean(), 2) if not sp_df.empty else 0,
            'sp_div_min': sp_df["SP_Return_Dividend"].min() if not sp_df.empty else 0,
            'sp_div_max': sp_df["SP_Return_Dividend"].max() if not sp_df.empty else 0,
            'sp_div_mean': round(sp_df["SP_Return_Dividend"].mean(), 2) if not sp_df.empty else 0
        }
        ...
        logger.error(f"Error in S&P Return calculation: {str(e)}")
        return render(request, 'spreturn/spreturn.html', {
...
def spreturn_insights(request):
    """
    View to display insights on S&P 500 returns across different time periods.
    Analyzes historical data to show min, max, and average returns.
    """
    logger.info("Accessing S&P Return insights")
    ...
    try:
        ...
        if not sp_info_queryset.exists():
        ...
        # Check if we have any summary data
        if not spreturn_summary:
            logger.warning("No valid summary data could be calculated")
            context = {
                'error': "Not enough data to calculate meaningful insights."
            }
            return render(request, 'spreturn/sp_insights.html', context)
        ...
        logger.error(f"Error in S&P Return insights calculation: {str(e)}")
        return render(request, 'spreturn/sp_insights.html', {
            'error': f'An error occurred: {str(e)}'
        })
